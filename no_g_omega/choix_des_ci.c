...
struct Revenu rcdn(float C1, float C2, float Qf1, float Qf2, struct Market market, struct CDN cdn, float p1, float p2, struct Coeffs coeff, float qc, float qf)
{
    ...
    if (coeff.coeff_1 != 0)
    {
        if (coeff.coeff_2 != 0)
        {
            float B1 = (Q1/pow(p1, 2))+((Q1+Q2)*((1-market.omeg)/2)/pow(p1+p2, 2));
            M1 = fmaxf(p1*B1/(1+B1+(Q2/pow(p2, 2))), 0);
        }else{
    ...
    if (coeff.coeff_2 != 0)
    {
        if (coeff.coeff_1 != 0)
        {
            float B2 = (Q2/pow(p2, 2))+((Q1+Q2)*((1-market.omeg)/2)/pow(p1+p2, 2));
            M2 = fmaxf(p2*B2/(1+B2+(Q1/pow(p1, 2))), 0);
        }else{
...
int main()
{
    ...
    cdn.request_price = 5; // valeur arbitraire
